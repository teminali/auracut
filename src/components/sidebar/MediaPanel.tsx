    ...
    >
      <div className="panel-header">
        ...
        <div className="flex items-center gap-1">
          ...
          <button onClick={() => fileInputRef.current?.click()} className="editor-library-action h-[var(--h-sm)] px-2 gap-1.5 text-ui-xs font-semibold inline-flex items-center rounded-squircle-sm">
      ...
      <div className="px-2 py-2 flex-shrink-0">
        <div className="pro-input flex items-center gap-2 px-2 h-[var(--h-md)]">
...
const AssetRow: React.FC<{
  ...
  return (
    <div
      ...
      className="card-interactive relative p-[7px] flex items-center gap-2 group"
    ...
    >
      ...
      <div className="w-[54px] h-[34px] rounded-squircle-xs bg-black overflow-hidden flex-shrink-0 flex items-center justify-center relative">
        ...
        {asset.type === 'audio' && (
          <span className="absolute inset-0 bg-lane-audio/20 flex items-center justify-center">
            <Music className="w-4 h-4 text-spectrum-textBright" />
          </span>
        )}
      </div>
      ...
      <div className="flex items-center gap-0.5 flex-shrink-0">
        <button
          ...
          className="btn-ghost-danger w-[22px] h-[var(--h-xs)] absolute top-[5px] right-[5px] z-10 opacity-0 group-hover:opacity-100 transition-opacity"
          title="Remove from pool"
        
            aria-label="Remove from pool">
          <Trash2 className="w-3 h-3" />
        </button>
        <span className="w-[22px] h-[var(--h-xs)] rounded-squircle-sm bg-line-bright group-hover:bg-spectrum-accent text-spectrum-onAccent group-hover:text-spectrum-onAccent flex items-center justify-center transition-colors">
  ...
  return (
    ...
    >
      <div className="aspect-video bg-black/60 relative overflow-hidden flex items-center justify-center">
        ...
        <span className="absolute bottom-1 right-1 px-1 rounded-squircle-2xs bg-black/80 text-micro font-mono text-spectrum-textMuted tabular leading-[14px]">
