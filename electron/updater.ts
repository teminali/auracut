...
export const RELEASES_URL = 'https://github.com/teminali/teminaliCut/releases/latest';
...
export async function listReleases(limit = 4): Promise<ReleaseOption[]> {
  ...
  const res = await fetch(
    'https://api.github.com/repos/teminali/teminaliCut/releases?per_page=15',
...
async function sideloadUpdate(version?: string): Promise<SideloadResult> {
  ...
  try {
    const base = version
      ? `https://github.com/teminali/teminaliCut/releases/download/v${encodeURIComponent(version)}`
      : 'https://github.com/teminali/teminaliCut/releases/latest/download';
